...
The app under test is the real one, assembled by ``build_app`` with its whole
middleware stack, mounted over an empty Starlette in place of the MCP app.
That mount is what PR 2.2 turns into ``create_app(settings, mcp_app=None)``:
the REST surface has to stand up without an MCP server behind it, and this is
where that starts being true.
"""
...
_spec = importlib.util.spec_from_file_location("hf_app_goldens", _APP_PATH)
...
class _FrozenMeta(type):
    ...
    ``_to_json`` narrows on ``isinstance(obj, datetime)`` against the module
    global we are replacing. Without this, every real datetime in a report
    would fail that check, fall through to the passthrough branch, and blow up
    in ``json.dumps``.
    """
...
def deterministic_world(monkeypatch):
    ...
    monkeypatch.setattr(app, "datetime", _frozen_datetime(FROZEN_NOW))
...
def client():
    ...
    """
    return TestClient(app.build_app(Starlette()))
...
@pytest.fixture
def fixture_atlas(tmp_path, monkeypatch):
...
class TestMarineOverlay:
    ...
    def test_covered_point(self, client, fixture_atlas) -> None:
        resp = client.get(
            "/api/v1/marine/marc",
            params={
                "lat": BREST[0],
    ...
    def test_coverage(self, client, fixture_atlas) -> None:
        resp = client.get("/api/v1/marine/marc/coverage")
        assert resp.status_code == 200

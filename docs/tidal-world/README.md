...
| **NOAA OFS** (CBOFS, DBOFS, TBOFS, GoMOFS, NGOFS2, SFBOFS, SSCOFS, CIOFS, WCOFS, lacs) [vérifié live S3, spike SSCOFS du 2026-09-21] | baies et façades US | 10 m à 14 km selon système (SSCOFS 163 m médian, SFBOFS 104 m) | PG 3D horaire, marée incluse ; NetCDF-4 non compressés, la surface se lit par requêtes Range | analysé (SSCOFS, 32 j, validé contre 8 stations CO-OPS) | S et 3D | S3 `noaa-nos-ofs-pds` sans clé, THREDDS | `noaa-nos-ofs-pds` depuis 2022-01 (SSCOFS depuis 2024-09) | NODD, `registry.opendata.aws/noaa-ofs/` : « can be used as desired » | oui | OK, 210 Mo par fichier de champs SSCOFS, 5 Mo lus par heure |
...
La même couleur, « couvert, marée non significative », couvre ensuite tout ce qu'un modèle **servi** calcule sans marée, **sans rien télécharger** : les atlas ne gardent que les cellules à 0,2 kt ou plus, donc le domaine mouillé du modèle moins ses cellules gardées est exactement là où il a trouvé moins que ça. `scripts/build_tidal_domain.py` lit ce domaine dans le masque terre-mer d'un seul fichier brut par modèle (`map/domain_<modèle>.geojson` : Copernicus MED, IBI, NWS, Arctique Islande, les huit boîtes NorKyst), et `negligible_tide()` en retire les étendues de cellules des masques servis puis la terre. Résultat du 2026-09-23 : 421 degrés carrés, la Méditerranée ouverte, le Kattegat, les fjords norvégiens calmes, le large islandais, plus la Baltique mesurée. Fichier web `negligible.geojson`, 83 ko compressés.
...
à 90 m). Sur la carte, une passe dont la meilleure source servie est aveugle
n'est plus comptée comme couverte : Saltstraumen repasse en rouge sous
NorKyst, Lundy et les Shoots en violet sous MANGA.

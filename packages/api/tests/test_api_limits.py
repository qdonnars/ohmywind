...
@pytest.fixture
def client():
    return TestClient(create_app(Settings(), _StubMcpApp()))
...
def test_the_app_starts_and_stops_with_the_coverage_warm_up() -> None:
    ...
    """
    with TestClient(create_app(Settings(), _StubMcpApp())) as client:
...
def test_the_warm_up_survives_a_broken_registry(monkeypatch) -> None:
    ...
    app = create_app(Settings(), _StubMcpApp())
    # ``Services`` is frozen, so the whole holder is swapped rather than one
    # of its fields. The lifespan reads it off app.state, like every handler.
    monkeypatch.setattr(app.state, "services", SimpleNamespace(marc=_BrokenRegistry(), shom=None))

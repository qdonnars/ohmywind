...
**What is covered, what is missing.** The map below answers three questions: where tidal current matters (areas over 0.5 and 1.5 kt computed from the ATLNE atlas, completed by the known passes and races), what the app uses today and at which precision, and what is missing in the target area (Europe): the strong-current places without a fine or medium source. A click on the sea asks the server and shows the source it would pick at that point. The candidate sources and their licences sit in the registry, folded by default: not all of them are open data, and that is what paces the extension of the coverage.
